...
export default createStore({
  ...
  getters: {
    // quartosHotel: (state) => {
    //   let quartosHotel = state.quartosHotel.map((item) => {
    //     return {
    //       id: item.id,
    //       img: item.img,
    //       title: item.title,
    ...
    servicosAdicionais: (state) => {
      let servicosAdicionais = state.servicosAdicionais.map((item) => {
        return {
          id: item.id,
          name: item.name,
          price: item.price,
  ...
  mutations: {
    ...
    // Direciona quarto escolhido para primeira posição em Reservas
    updateCards (state, quartoHotel) {
      const index = state.quartosHotel.findIndex(c => c.id === quartoHotel.id)
      if (index === -1) {
        state.quartosHotel.unshift(quartoHotel)
      } else {
        state.quartosHotel.splice(index, 1)
        state.quartosHotel.unshift(quartoHotel)
      }
    }
  },
